...
public class ReceivingGoodEE extends Behaviour {
    int onEnd = 1;
    ...
    public void action() {
        MessageTemplate reply = MessageTemplate.and(
                ...
                MessageTemplate.MatchProtocol(Protocols.IBoughtEnergy));
        ACLMessage myReply = getAgent().receive( reply );
        if ( myReply != null) {
            System.out.println(getAgent().getLocalName()+" получил ЭЭ");
            onEnd = 2;
        }
    }
