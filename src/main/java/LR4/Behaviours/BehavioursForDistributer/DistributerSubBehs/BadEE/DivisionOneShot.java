...
public class DivisionOneShot extends OneShotBehaviour {
    NeedsData needsData;
    private Services topicname;

    public DivisionOneShot(Agent a, NeedsData needsData) {
        super(a);
    ...
    public void action() {
        ...
        needs.setContent(String.valueOf(needsData.getkWh()/3));
        for (AID agent : agents) {
            needs.addReceiver(agent);
        }
        needs.setProtocol(Protocols.LetsStartTrade);
        getAgent().send(needs);
        System.out.println(getAgent().getLocalName()+" попробуем разделить ЭЭ");
